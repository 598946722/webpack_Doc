...
```
因为nodeJS和ES6的模块加载机制不同，‘commonjs’ 的 require 和 import 的区别是  requre需要等页面加载完才进行导入，而import则是立即加载，使得一部分通过npm下载直接拿来用的loader或者插件会直接导致样式或者js加载异常，渲染顺序有误，这边在进行项目开发的时候还是需要稍微注意下的。
```
